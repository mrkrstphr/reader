...
function useFetchPages(issue, page) {
  ...
  useEffect(() => {
    if (issue && page <= issue.pageCount) {
      fetch(`/issue/${issue.id}/page/${page}`).then(body => body.blob()).then(img => URL.createObjectURL(img)).then(setCurrentPage);
    }
  }, [issue, page]);
...
function useFetchThumbnail(issue, page) {
  ...
  useEffect(() => {
    if (issue && page <= issue.pageCount) {
      fetch(`/issue/${issue.id}/page/${page}/thumbnail`).then(body => body.blob()).then(img => URL.createObjectURL(img)).then(setThumbnail);
    }
  }, [issue, page]);
...
function Thumbnail({ issue, onClick, page }) {
  ...
  return thumbnail ? <img alt={`Thumbnail ${page}`} src={thumbnail} className="h-36 cursor-pointer" onClick={onClick} /> : null;
}
...
export default function IssueReaderPage() {
  ...
  return (
    <div><h1>ISSUE READER VIEW</h1>
    {/* <pre>{JSON.stringify(issue, null, 2)}</pre> */}

    <div>
      <button className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" type="button" onClick={() => {
        setPage(Math.min(page + 1, issue.pageCount));
      }} disabled={isNil(issue) || page === issue.pageCount}>Next</button>
    </div>
    ...
    <div class="fixed z-10 inset-0 overflow-y-auto"
      ...
      <div class="flex items-end justify-center min-h-screen text-center">
        ...
        >
          {currentPage && <img alt="" src={currentPage} className="max-h-screen max-w-screen m-auto select-none" />}
          {isOverlayActive && (
            <div className="absolute top-0 left-0 mt-4 ml-4 w-full flex items-end">
              <Icon icon="times" className="text-white cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" onClick={e => {
                e.stopPropagation();
                history.push(`/issue/${id}/details`);
              }}
              />
          ...
          {isOverlayActive && (
            <div className="bg-black bg-opacity-80 text-white p-2 absolute w-full bottom-0 z-40">
              <div>
                <span>{issue ? <>Page {page} / {issue.pageCount}</> : '&nbsp;'}</span>
              </div>
            </div>
          )}
        </div>
